	...
	public function save_inputs( \WP_REST_Request $request ) {
		...
		if ( empty( $data ) ) {
			return new \WP_REST_Response(
				array(
					'code'    => 'error',
					'message' => esc_html__( 'White Label error: The form data was not sent.', 'textdomain' ),
					'markup'  => esc_html__( 'It seems that there is an error. The options were not saved.', 'textdomain' ),
				),
				400
			);
		}
		...
		return new \WP_REST_Response(
			array(
				'code'    => 'success',
				'message' => esc_html__( 'Options saved.', 'textdomain' ),
			),
			200
		);
	}
